        ...
        <form act-on="submit" act-request="{{ route('mobile.signin.request', ['return' => $return]) }}">
            <div class="section  mb-5">
                ...
                <div class="form-links mt-2">
                        <div>
                            <a href="{{ route('mobile.signup') }}">{{ __('Register Now') }}</a>
                        </div>
                        <div><a href="{{ route('mobile.password.reset') }}">{{ __('Forgot Password?') }}</a></div>
                    </div>

                <div class="appBottomMenu">
                    <button type="submit" class="btn btn-primary btn-block btn-lg">{{ __('Continue') }}</button>
                </div>
            </div>
        </form>
    </div>
