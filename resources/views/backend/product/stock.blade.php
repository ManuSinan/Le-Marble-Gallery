    ...
    <div class="col-md-4 col-sm-12">
        <div class="mb-1">
            <ol class="breadcrumb breadcrumb-alternate" aria-label="breadcrumbs">
                ...
                <li class="breadcrumb-item active" aria-current="page"><a href="javascript:;">{{ request('title') ?? __('Limited Stock Books') }}</a></li>
            </ol>
        </div>
        <h2 class="page-title" act-on="click">{{ request('title') ?? __('Limited Stock Books') }}</h2>
    </div>
            ...
            <div class="filter search">
                <div class="input-icon">
                    <form act-on="submit" action="/">
                        <input type="search" id="search" class="form-control" value="{{ $search }}" placeholder="Search {{ __('Books') }}">
            ...
            <table act-sort='[[ 4, "asc" ]]' act-datatable="{{ route('product.stocklist',[ 'max' => request('max') ]) }}" search="#search" class="table card-table table-vcenter text-nowrap datatable dtlink">
                <thead>
                    <tr class="bg-transparent">
                        ...
                        <th name="name" priority="4">Book Name</th>
                        <th name="category_name" priority="5">Category</th>
                        <th name="product_code" priority="2">Book Code</th>
